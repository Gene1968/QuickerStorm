...
| 15 | ✅ **Load-governor / render decouple (Phase 1) — 2026-06-21, live-verified on Aspen, UNCOMMITTED (Gene commits)** | Root cause of the "Aspen used to load but now stalls at a 32m bubble": the fps-driven `_renderCap` gated the BUILD radius, so load-stutter (build/ingest/cull longtasks, NOT render cost) floored the scene to 32m while most RAM sat idle (Auto reload: heap 27%, buildQ 21701 frozen, throttle=`brkCap` not `gov`). FIX: (a) DELETED `_renderCap` — build/render no longer gated by frame rate; `renderRadius()` = user draw distance; (b) NEW pure `cullPolicy.shouldEvictForHeap` (evicts only when heap≥0.92 AND appRatio≥0.85 = resident genuinely explains the heap — does NOT fire in the heap-high/app-low transient-garbage regime that 2026-06-18 proved eviction can't relieve), wired into the cull `over` trigger so a raised budget settles gracefully instead of wedging; (c) resident budget raised `APP_BUDGET_CAP 2048→3072` / `FRACTION 0.50→0.65` to use available RAM. LIVE RESULT: dd=192m (full), build flows 891–1055/s, objs 3276→~11k, `[Cull]` eviction arms + steps dd 192→176m under budget — no wedge, no crash. Files: cullPolicy.js (+test), useWorldEngine.js, memGovernor.js (+test). 246 lib tests + build:prod green. Spec/plan: docs/superpowers/{specs,plans}/2026-06-21-load-governor-render-decouple*. EXPOSED (as designed) = **render-bound at 192m: fps 11–20 on ~11k objs; settles ~11k/24858 at the ~2900MB heap-usable ceiling → the FULL region needs LOD (Phase 2, NEXT).** Two follow-on bugs surfaced (both low-fps/main-thread-saturation symptoms LOD should relieve): **(i) springback** — avatar position snap-back, AgentUpdate/PacketAck starved at 11–20fps; **(ii) slow re-stream while browsing** — moving brings a few hundred new in-range objects that take MINUTES to build (drain starved by low fps + eviction churn at the budget ceiling). |
...
- [~] **⭐ Off-main-thread cache I/O worker — ROOT-CAUSE FIX for heavy-region "warm loads cold" (#10/#11/#13), BUILT + LIVE-VERIFIED 2026-06-21, UNCOMMITTED.** Root cause was main-thread saturation starving IDB callbacks → `geomCacheGetMany` degraded to all-miss → re-bake spiral. Fix: moved qs-geom IDB + big mem-tier into a Web Worker (`geomCacheCore.js`/`cacheIO.worker.js`/`useCacheIO.js`; `geomCache.js` now thin L1+worker-client+sync-fallback; `uiStore.cacheWorker` kill-switch); engine waits for the worker's hit/miss verdict (removed the 4s re-bake watchdog). PLUS a **frame-time draw-distance cap** (`_renderCap`) so render can't saturate main regardless of free memory, + main-thread `persist()` + no-worker-recycle so writes persist across reloads. LIVE: warm Aspen served `idb=255-311` cache hits, frame cap auto-tuned dd 32→96m at 54-60fps, no brick. Spec/plan `docs/superpowers/{specs,plans}/2026-06-21-cache-io-worker*`; mem [[idb-mainthread-starvation-rootcause]]. **NEXT (exposed ceiling): LOD** — a 24k-obj region is GPU-render-bound (even ~96m is the sustainable bubble); render the far field cheaply to see the whole region. Also OPEN: Phase 2 = move qs-mesh off-thread (meshes still grid-refetch); optional worker write-buffer tuning (idb dips while baking uncached parts).
...
- [ ] **512m VAR-REGION bugs on "Aspen Homesites" (2026-06-18, Gene live) — position snaps to Y=255 + terrain missing.** Three symptoms, all var-region-specific (region is 512×512; clamps/terrain assume 256): **(a) intra-region TP snaps to Y=255** — ✅ FIXED 2026-06-18 (UNCOMMITTED): `server/handlers/lludp.ts` `C.MAP_TELEPORT` handler hardcoded `min(255,d.x/y)`; raised to the **8191** var-region sanity bound to match the `C.TELEPORT` handler above it (which was already fixed — MAP_TELEPORT was simply missed). NEEDS live-verify (reconnect, intra-TP past Y=255 lands correctly). **(b) WALKING past the half-region line snaps back** — ✅ ROOT-CAUSED + FIXED 2026-06-19 (UNCOMMITTED, needs live-verify). Located: `useWorldEngine.js` dead-reckon clamps the avatar each frame to `[1, sessionStore.regionSizeX/Y - 1]`. `regionSize` was STALE at the LOGIN region's value (512) — RegionHandshake carries no size, the EQ/UDP `TeleportFinish` decoder DROPPED `RegionSizeX/RegionSizeY`, `EnableSimulator` was unhandled, and the client map-block backfill is async/fragile (it failed) → in a 1024 region the clamp walled the avatar at Y=511 (camera, unclamped, panned freely — the tell). FIX: `server/lib/eventQueue.ts` `decodeTeleportFinishLLSD` now decodes `RegionSizeX/RegionSizeY`; `applyTeleportFinish` forwards them in `S.TELEPORT_FINISH`; client `onTeleportFinish` applies them authoritatively (else keeps the map-block fallback). `EnableSimulator` now logs its size (instrumentation). 5 EQ tests + 152 server + 241 lib + build:prod green. Spec `docs/superpowers/specs/2026-06-19-varregion-size-on-tp-design.md`; mem [[varregion-size-on-teleport]]. ⚠ CORRECTION (Gene live 527/574, sees full hills/rivers NE): VISIBLE terrain past 512 WORKS — the live `onTerrainPatch` render reads payload heights bounded by the real regionSize (`vStride=rx+1`, guard `slX>rx`). The `TERRAIN_STRIDE=513` / `setTerrainPatch >=512 continue` cap only limits the worldStore PERSISTENCE copy → affects `sampleTerrainHeight` (avatar local ground prediction past 512, but sim-corrected/minor) and `rebuildTerrainFromStore` after an HMR remount (SW-quadrant only, transient). NOT a visible bug; no rewrite needed. Optional tidy: scale stride+guard to regionSize so persistence/sampling are var-region-correct. **(c) terrain disappeared** (after hard reload, and again after Resync World) — server log shows Aspen receiving ONLY `[terrain] WATER_FLOOR patches=2` (0x37) — NO LAND patches (a 512 region should send a 32×32 land grid); land-terrain decode/arrival for this var region is producing nothing → flat/absent terrain. Relates to [[var-region-terrain-fix]] (LandExtended 0x4D, 32-bit patch IDs) + [[var-region-map-fixes]]. NOT caused by today's heap/badge changes (those don't touch terrain or position clamps; heap fix makes eviction LESS aggressive). The "stuck at 543 remaining textures" Gene saw first was on the pre-reload OLD bundle (cleared by hard reload). NEXT: fix (a) [small — regionSize clamp], trace (b), investigate (c) land-patch decode on 512 regions.
...
- [~] **Drop-through-floor (recurring, big) (2026-06-13)** — avatar falls through terrain. **VAR-REGION CAUSE ROOT-CAUSED + FIXED + LIVE-VERIFIED 2026-06-20 (UNCOMMITTED):** on a 1024 var-region the avatar fell to z=1 in ~¾ of the region (could only fly). NOT the 0x37 theory (a `[TerrainDiag]` probe disproved it: 0x37 decodes to coord (0,0) + is server-filtered). REAL cause: the COLLISION heightmap `worldStore.terrainHeights` was a FIXED 513² array (covers 0–512 only) while the RENDER reads patch payloads full-region — so past 512 `sampleTerrainHeight`→≈0 (collisionH=0.00 vs renderH≈39, proven live) → gravity floored the avatar at z=1, sim yanked it back. FIX: region-size the collision heightmap — `worldStore.ensureTerrainGrid(regionDim)` reallocates `terrainHeights` to `(regionDim+1)²` (pure `resolveTerrainStride`, cap MAX_TERRAIN_DIM=2048); dynamic `TERRAIN_STRIDE`/`setTerrainPatch` bound; `onTerrainPatch` calls it before storing. LIVE-VERIFIED: collisionH≈renderH past 512, stride=1025, walks NE hills at correct z. 6 vitest + 58 store tests + build:prod green. mem [[varregion-size-on-teleport]]. ⚠ NOTE: a standard-256-region drop-through (the original 0x37 clue, avatar z≈22) may be a SEPARATE residual — re-confirm on a 256 region; this fix addresses the var-region (>512) cause.
...
- [ ] flexi, flutter anim?
- [~] Particle systems (PSBlock) — ⭐ v1 IMPLEMENTED 2026-06-20 (UNCOMMITTED, telemetry-verified live; visual confirm pending). Full pipeline: bounded decode `server/lib/particleCodec.ts` (layout from Firestorm `llpartdata.cpp`, fixes the old tail-OOB) → `psys` on obj_upd → pure FS-faithful sim `src/lib/particleSim.js` (DROP/EXPLODE/ANGLE/ANGLE_CONE, age color/alpha/scale, additive vs alpha) → `src/composables/useParticles.js` (THREE.Points pool, runtime radial-sprite fallback + real texture via cache, GLOBAL_PARTICLE_CAP=20k/PER_EMITTER_CAP=512, 96m distance cull, heap-aware) → wired in useWorldEngine (register/unregister/step/telemetry). Emitter follows the mesh WORLD position (linkset-child fix). Live: server `[PSys]` decode lines + client `[Drain] ps=E/L in=I near=Dm`; peak 49 live confirmed when within 96m. Specs/plan: docs/superpowers/{specs,plans}/2026-06-20-particle-systems*. Built subagent-driven (T1–T8). OPEN: visual eyeball on a light region; deferred sim flags (target/beam/ribbon, wind, bounce); 96m cull is a tunable. mem [[particle-systems-feature]]
- [!] **`ObjectUpdateCompressed` particle path — IMPLEMENTED 2026-06-21 (UNCOMMITTED, NEEDS LIVE-VERIFY).** Root cause of the "pastel-plane, no particles" emitters (Gene's localId 955628718 / 955628792): `decodeObjectUpdateCompressed` (lludp-codec.ts) marked any prim with CompressedFlags `0x08 HasParticlesLegacy` / `0x400 HasParticlesNew` as `RARE` and **bailed the whole conditional zone** → never reached TextureEntry → NO `defaultColor`/`defaultTexture` AND no `psys` → pastel hash + no emitter. Fix (verified against OpenSim `LLClientView.CreateCompressedUpdateBlockZC`): narrowed `RARE` to `0x01|0x02` (ScratchPad/Tree only); decode the **legacy** block as a fixed **86 B** between MediaURL and ExtraParams (consuming exactly 86 keeps the following shape/TE aligned — the desync risk v1 feared), and the **new** block (self-describing, runs to dataEnd) right after TE+TexAnim. `psys` now emitted from the compressed path too. Blast radius is one object regardless (each object is Variable2-length-framed; `off = dataEnd` resets per object). Tests: server suite 174/0 incl. new `server/__tests__/compressed-particles.test.ts` (3 cases: legacy decode + TE-alignment desync guard, new decode, two-object cross-desync guard); real captured fixtures unchanged. LIVE-VERIFY: on the region with those two emitters, expect them to (a) emit particles and (b) render with their real TE (likely transparent emitter prim) instead of a pastel plane. Pairs with the cosmetic pastel→white fallback (useWorldEngine.js, same session). mem [[particle-systems-feature]]
