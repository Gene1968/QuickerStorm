...
Today: per-UUID jellydoll capsule + tube placeholder; AvatarAppearance decoded (bakes cached, not composited); **rigged mesh now skins to the avatar at rest pose (AV-1)** — but only single rigged meshes place cleanly; linkset child prims scatter, no cohesive body placeholder yet, facing 90° off. No animation.
...
- ✅ **AV-1 rigged-mesh rest-pose skinning** — shipped 2026-07-17, live-confirmed. Server decodes the mesh skin block (bind_shape + inverse_bind + per-vertex Weights) + bundles the SL default skeleton (bones **and** collision volumes, for fitted/"Liquid" mesh) and bakes REST-POSE skinned positions; client places the mesh at the avatar root (`RIG_FOOT_OFFSET`). Single rigged meshes (pants/shoe/hair/body) render correctly on the avatar. (→ CHANGELOG). **AV-1 followups (still scattered/not user-ready):**
  ...
  - 🔜 **Avatar facing 90° off** — our tube + controls/cam face **East** but the mesh avatar / FS user face **South**. Align avatar facing to match FS **without** breaking the input-driven controls/cam (the capsule face-indicator uses −Z-forward vs the rigged-mesh/SL +X-forward — reconcile the convention).
