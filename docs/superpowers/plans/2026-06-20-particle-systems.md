...
1. **`ObjectUpdateCompressed` particle path** — v1 decodes PSBlock only in the full `ObjectUpdate`. The compressed decoder (`lludp-codec.ts` ~1093–1280) packs PSBlock conditionally behind a compressed-flag bit; add the same `decodeParticleSystem` call there once v1 is verified.
