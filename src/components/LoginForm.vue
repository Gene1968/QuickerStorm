...
<template>
  <form class="flex flex-col gap-3" @submit.prevent="submit">
    ...
    <div class="flex flex-col gap-1.5">
      <label class="text-t2 text-xs uppercase tracking-wide">Start Location</label>
      <div class="flex gap-2">
        <select
          v-model="destType"
          class="flex-1 px-2 py-1.5 rounded bg-card border border-brd text-t1 text-sm focus:outline-none focus:ring-1 focus:ring-accent"
        >
    ...
    <button
      type="submit"
      class="px-4 py-2 rounded bg-accent text-white font-semibold hover:opacity-80 disabled:opacity-50 transition-opacity"
