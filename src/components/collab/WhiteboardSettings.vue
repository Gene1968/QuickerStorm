<script setup>
/**
 * WhiteboardSettings — Owner settings panel for the whiteboard.
 * Shows current user as owner (non-removable), lists other members with
 * role management (owner/editor/viewer/remove), and allows archiving.
 */
...
const emit = defineEmits(['update', 'archive', 'set-locked', 'close'])
...
// Initialize members from current server state, resolve names
const members = ref(
	props.currentMembers
		.filter(m => m.userId !== avatarStore.authUserId) // exclude self
		.map(m => {
...
// Count owners (including self) to determine if self can be demoted
const ownerCount = computed(() => {
	let count = 1 // current user is always owner here
...
// All users except current user and already-added members
...
const availableUsers = computed(() => {
	return presenceStore.users
		.filter(u => {
			...
			return uid
				&& uid !== myAuthId.value
				&& !members.value.find(m => m.userId === uid)
		})
...
async function handleArchive() {
	const ok = await avaConfirm({
		title: 'Archive Board',
		message: 'Archive this whiteboard? It will be saved to history and a new board can be created.',
	...
	<div class="settings-panel">
		...
		<div class="settings-body">
			...
			<template v-if="editAccess === 'private'">
				...
				<!-- Current user (always owner, non-removable) -->
			...
			<template v-if="!archived">
				<button class="btn-danger" @click="handleArchive">🔒 Archive Board</button>
				<p class="hint">Saves a snapshot to history (read-only). The whiteboard will be cleared for everyone. You can re-enable editing later from this panel.</p>
			</template>
			<template v-else-if="locked">
				<button class="btn-secondary action-btn" @click="emit('set-locked', false)">🔓 Allow editing</button>
				<p class="hint">Unlocks this archived whiteboard so members can edit it again.</p>
			</template>
			<template v-else>
				<button class="btn-danger action-btn" @click="emit('set-locked', true)">🔒 Lock (read-only)</button>
				<p class="hint">Locks this archived whiteboard so it becomes read-only.</p>
			</template>
		</div>
	</div>
</template>
