<script setup>
/**
 * CollabDocSettings — Owner settings panel for a collaborative doc.
 * Mirrors WhiteboardSettings (title + access + members + archive).
 */
...
async function handleArchive() {
	const ok = await avaConfirm({
		title: 'Archive Doc',
		message: 'Archive this document? It will be saved to history and a new doc can be created.',
	...
	<div class="settings-panel">
		...
		<div class="settings-body">
			...
			<template v-if="!archived">
				<button class="btn-danger" @click="handleArchive">🔒 Archive Doc</button>
				<p class="hint">Saves a snapshot of the current doc to history (read-only). The doc will be cleared and closed for everyone. You can re-enable editing on archived docs from this panel later.</p>
			</template>
			<template v-else-if="locked">
				<button class="btn-secondary action-btn" @click="emit('set-locked', false)">🔓 Allow editing</button>
				<p class="hint">Unlocks this archived doc so members can edit it again.</p>
			</template>
			<template v-else>
				<button class="btn-danger action-btn" @click="emit('set-locked', true)">🔒 Lock (read-only)</button>
				<p class="hint">Locks this archived doc so it becomes read-only.</p>
			</template>
		</div>
	</div>
</template>
...
.member-row {
	...
	padding: 6px 10px; background: #f1f5f9; border-radius: 6px; margin-bottom: 4px;
}
