...
const REGION_MATURITY = {
	...
	254: { label: 'Offline 💤',  color: 'text-fg-muted'         },
}
...
<template>
	<div class="locationbar relative flex items-center gap-2 rounded-sm bg-white/10 ps-3 text-xs text-fg select-none min-w-0">

		<span @click="showLocationInfo" title="See more info about the current location (TO-DO)" class="me-2 text-base">ℹ️</span>

		<!-- Connection status dot -->
		<span class="fs-5 shrink-0 leading-none" :class="connected ? 'text-green-400' : 'text-red-400'" :title="'You are ' + (connected ? 'online' : 'disconnected')">
			{{ connected ? '●' : '○' }}
		</span>
		...
		<template v-if="!editing">
			<button
				class="flex items-center gap-1 min-w-0 text-left hover:bg-white/10 rounded-sm -mx-1 px-1 transition-colors"
		...
		<template v-else>
			<input
				ref="editInput"
				v-model="editVal"
				class="flex-1 bg-white/10 border border-accent/50 rounded-sm px-2 py-0.5 min-w-[32rem] text-xs font-mono text-fg focus:outline-hidden focus:ring-1 focus:ring-accent"
				@keydown="onEditKeydown"
				@blur="cancelEdit"
			/>
			<span class="text-fg text-xs shrink-0">Esc or Enter</span>
		</template>

		<span class="mx-3"></span>
		<MicVocalIcon v-if="false" title="Voice (TO-DO)" class="w-4 h-4 me-1 text-gray-400" />
		<BirdIcon v-if="false" title="Flying (TO-DO)" class="w-4 h-4 me-1 text-gray-400" />
		<SwordIcon v-if="false" title="Pushing (TO-DO)" class="w-4 h-4 me-1 text-gray-400" />
