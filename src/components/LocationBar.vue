...
<template>
	<div class="flex items-center gap-2 bg-white/10 rounded-sm ps-3 text-xs text-fg select-none min-w-0">
		...
		<template v-else>
			<input
				ref="editInput"
				v-model="editVal"
				class="flex-1 bg-white/10 border border-accent/50 rounded-sm px-2 py-0.5 min-w-[35rem] text-xs font-mono text-fg focus:outline-hidden focus:ring-1 focus:ring-accent"
	...
	<div v-if="showLocationHistory" class="absolute top-8 right-0 bg-black/50 w-[33.333rem] translate-x-[-130%] p-1 text-sm text-fg z-10">
