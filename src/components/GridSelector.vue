...
<template>
  <div class="flex gap-2">
    <select
      class="flex-1 px-3 py-2 rounded bg-card border border-brd text-t1 focus:outline-none focus:ring-2 focus:ring-accent"
    ...
    <button
      ...
      class="px-2 py-2 rounded border border-brd text-t2 hover:text-red-500 hover:border-red-500 transition-colors"
    ...
    <a
      ...
      class="px-2 py-2 rounded border border-brd text-t2 hover:text-accent transition-colors text-sm"
      title="Grid website"
    >↗</a>
  </div>

  <!-- Grid detail hints (register / forgot password) -->
  <div v-if="store.selectedGrid" class="flex gap-3 mt-1 text-xs text-t2">
    <a
