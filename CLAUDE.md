...
- **Complete the feature across surfaces.** When you learn what a feature needs, wire every surface it belongs on in the same pass: MenuBar + `ObjectContextMenu`/`AvatarContextMenu`/`LandContextMenu` + `InventoryContextMenu` + the relevant floater. No new disabled "(to-do)" control without a dated entry in the right bundle of `docs/ROADMAP.md`.
